...
def fetch_cpcb_data() -> pd.DataFrame:
    ...
    if not api_key:
        raise RuntimeError("CPCB_API_KEY environment variable not found.")
    ...
    if response is None:
        raise RuntimeError("CPCB API request failed.")

    try:
        df = pd.read_csv(io.StringIO(response.text))
    except Exception as exc:  # pragma: no cover - defensive logging
        raise RuntimeError(f"Failed to parse CPCB data: {exc}") from exc

    if df.empty:
        raise RuntimeError("CPCB API returned empty data.")

    # Defensive: ensure the expected columns exist before filtering
    for expected in ("state", "city", "station"):
        if expected not in df.columns:
            raise RuntimeError(f"CPCB data missing column '{expected}'.")
    ...
    if delhi_df.empty:
        raise RuntimeError("No CPCB rows matched Delhi filters.")
